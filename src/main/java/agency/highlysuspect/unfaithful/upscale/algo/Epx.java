...
public class Epx extends Upscaler {
	...
	protected void writeUpscalePixels(Sampler in, NativeImage out, int outX, int outY) {
		...
		out.setPixelColor(outX    , outY    , (similar(c, a) && !similar(c, d) && !similar(a, b)) ? blend(p, a, 1f) : p);
		out.setPixelColor(outX + 1, outY    , (similar(a, b) && !similar(a, c) && !similar(b, d)) ? blend(p, b, 1f) : p);
		out.setPixelColor(outX    , outY + 1, (similar(d, c) && !similar(d, b) && !similar(c, a)) ? blend(p, c, 1f) : p);
		out.setPixelColor(outX + 1, outY + 1, (similar(b, d) && !similar(b, a) && !similar(d, c)) ? blend(p, d, 1f) : p);
	}
